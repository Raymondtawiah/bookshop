...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    ...
    <button onclick="openFreeBookModal({{ $book->id }}, '{{ $book->title }}')"
       class="block transform group-hover:-translate-y-2 transition-all duration-300" title="Download PDF">
    @else
    <a href="{{ route('product.show', $book->id) }}"
       class="block transform group-hover:-translate-y-2 transition-all duration-300">
    @endif
