...
@section('content')
            ...
            <div class="max-w-3xl mx-auto mb-8">
                <h1 class="text-2xl font-bold text-gray-900">Edit Webinar</h1>
                <p class="text-gray-500">Update webinar information</p>
            </div>

            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-8 max-w-3xl mx-auto">
                <form action="{{ route('admin.webinars.update', $webinar->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            name="webinar_link" 
                            id="webinar_link"
                            value="{{ old('webinar_link', $webinar->webinar_link) }}" 
                            required
                            class="w-full px-5 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
                            placeholder="https://zoom.us/j/1234567890?pwd=..."
                        >
                        <p class="text-sm text-gray-400 mt-1">This link is hidden from users and only shown to paid attendees.</p>
                        @error('webinar_link')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
