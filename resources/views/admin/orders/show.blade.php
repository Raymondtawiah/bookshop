                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-2 py-3 text-sm text-gray-900 whitespace-nowrap">₵{{ number_format(is_array($item) ? ($item['unit_price_ghs'] ?? $item['unit_price_usd'] ?? $item['unit_price'] ?? 0) : ($item->unit_price_ghs ?? $item->unit_price_usd ?? $item->unit_price ?? 0), 2) }}</td>
                                        <td class="px-2 py-3 text-sm text-gray-900 whitespace-nowrap">{{ is_array($item) ? ($item['quantity'] ?? 1) : ($item->quantity ?? 1) }}</td>
                                        <td class="px-2 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">₵{{ number_format((is_array($item) ? ($item['unit_price_ghs'] ?? $item['unit_price_usd'] ?? $item['unit_price'] ?? 0) : ($item->unit_price_ghs ?? $item->unit_price_usd ?? $item->unit_price ?? 0)) * (is_array($item) ? ($item['quantity'] ?? 1) : ($item->quantity ?? 1)), 2) }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
