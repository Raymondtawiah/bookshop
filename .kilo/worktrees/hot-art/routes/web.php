...
use App\Http\Controllers\Api\VisaChatController;
...
// Visa AI Assistant (public)
Route::middleware(['web'])->prefix('visa-chat')->name('visa-chat.')->group(function () {
    Route::post('start', [VisaChatController::class, 'start'])->name('start');
    Route::post('message', [VisaChatController::class, 'sendMessage'])->name('message');
    Route::get('history', [VisaChatController::class, 'history'])->name('history');
    Route::delete('session/{token}', [VisaChatController::class, 'endSession'])->name('end');
});
