...
class VisaTrainingController extends Controller
{
    ...
    public function chat(Request $request)
    {
        ...
        Log::info('Visa training chat request', [
            'session' => $sessionId,
        ...
        if (in_array(strtolower($userMessage), ['restart', 'start over', 'try again', 'begin new', 'new interview'])) {
            ...
            Log::info('Visa training restart', ['session' => $sessionId]);
        ...
        Log::info('Visa training history before request', [
            'session' => $sessionId,
            'step' => $step,
        ...
        if ($step >= $this->maxSteps) {
            Log::info('Visa training forcing evaluation', ['session' => $sessionId, 'step' => $step]);
            $reply = $this->visaInterviewService->getEvaluation($history);
            $completed = true;
        } else {
            Log::info('Visa training getting next question', ['session' => $sessionId, 'step' => $step]);
            ...
            Log::info('Visa training got reply', [
                'session' => $sessionId,
                'reply_length' => strlen($reply),
                'reply_preview' => substr($reply, 0, 150),
                'completed' => $completed,
            ]);
        }
