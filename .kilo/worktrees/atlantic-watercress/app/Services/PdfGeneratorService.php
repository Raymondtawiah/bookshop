...
class PdfGeneratorService implements PdfGeneratorInterface
{
    ...
    public function hasWordFile(Book $book): bool
    {
        ...
        $extension = pathinfo($book->book_pdf, PATHINFO_EXTENSION);
    ...
    public function generateFromWord(Book $book, Order $order): array
    {
        try {
            ...
            if (! $sourcePath || ! file_exists($sourcePath)) {
                return [
                    'success' => false,
                    'message' => "Word file not found for book: {$book->title}",
                    'pdf_path' => null,
                ];
            }
            ...
            $pdf->AddPage();
            $pdf->SetFont('helvetica', 'B', 18);
            $pdf->Cell(0, 10, $book->title, 0, true, 'C');
            $pdf->Ln(5);
            $pdf->SetFont('helvetica', '', 12);
            $pdf->MultiCell(0, 10, "Author: {$book->author}");
            $pdf->Ln(10);
            ...
            Log::info("PDF generated from Word successfully: {$outputPath}");

            return [
                'success' => true,
                'message' => 'PDF generated from Word successfully',
                'pdf_path' => "{$this->storagePath}/{$filename}",
            ];

        } catch (\Exception $e) {
            Log::error('PDF generation from Word failed: '.$e->getMessage());

            return [
                'success' => false,
                'message' => 'Failed to generate PDF from Word: '.$e->getMessage(),
                'pdf_path' => null,
            ];
        }
    }
